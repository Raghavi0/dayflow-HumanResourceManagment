...
export default function LeaveApprovals() {
  ...
  useEffect(()=>fetch('http://localhost:5000/api/leave').then(r=>r.json()).then(d=>setPending(d.filter(x=>x.status==='Pending'))),[]);
  const action = async (id,status) => { await fetch('http://localhost:5000/api/leave/'+id,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({status,approved_by:user?.id||1})}); window.location.reload(); };
