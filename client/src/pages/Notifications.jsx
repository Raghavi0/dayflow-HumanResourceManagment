...
export default function Notifications() {
  ...
  const markRead = async id => { await fetch('http://localhost:5000/api/notifications/'+id+'/read',{method:'PUT',headers:{'Content-Type':'application/json'}}); setItems(prev=>prev.map(i=>i.id===id?{...i,is_read:1}:i)); };
  return <div><h2 className="gradient-text">Notifications</h2><div className="glass-card">            <button className="btn-primary" onClick={async()=>{await fetch('http://localhost:5000/api/notifications/'+user.id+'/read-all',{method:'PUT',headers:{'Content-Type':'application/json'}});setItems(prev=>prev.map(i=>({...i,is_read:1})));
