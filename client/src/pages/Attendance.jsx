...
export default function Attendance() {
  ...
  useEffect(()=>fetch(API_URL + '/api/attendance').then(r=>r.json()).then(setData),[]);
  const [tab,setTab]=useState('daily');
  const submit = async () => { await fetch(API_URL + '/api/attendance',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(form)}); window.location.reload(); };
